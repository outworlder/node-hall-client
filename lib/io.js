...
var HallUserAgent = require('./user_agent');

var debug = require('debug')('hall:io');
...
var streamBaseURL = function(session) {
...
var streamWSURL = function(io, session) {
...
var IO = Module.extend({
	...
	initSocket : function() {
		var me = this;
		var headers = _.extend({}, HallUserAgent.getHeader(), {
		...
		request({
			...
			if (body) {
				...
				if (resp.headers && resp.headers['set-cookie']) {
					...
					var new_cookie = resp.headers['set-cookie'][0].split(';')[0];
	...
	onError : function(error) {
		var me = this;
		me.trigger('error', 'Connection Error:\n' + error.toString());
		me.connect();
	},
	...
	sendMessageToSocket : function(level,name,data) {
		...
		var msg = (name && data) ? level+'::/room:'+JSON.stringify(msg_obj) : level+'::/room' ;
	...
	connect : function() {
		...
		if (!me.connectRetryTimer) {
			me.connectTimeout = (me.connectTimeout * 1.25);
			me.connectRetryTimer = _.delay(_.bind(function() {
				me._connect(true);
			}, me), me.connectTimeout);
			me._connect();
		}
	...
	_connect : function(clearTimeout) {
			...
			debug('trying to connect...timeout currently is set to: ' + me.connectTimeout);
